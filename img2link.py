...
if suffix == ".xlsx":
    ...
    i_idx_input = st.number_input(
        "Image column index (1-based, the pictures column; 0 = auto)",
        min_value=0, max_value=len(headers), value=min(3, len(headers))
    )
    n_idx_input = st.number_input(
    ...
    i_idx = (i_idx_input - 1) if i_idx_input > 0 else None
    ...
    if st.button("Convert"):
        ...
        matched = sum(1 for r in range(len(df)) if (header_row + r) in embedded)
...
# ------------------------- CSV -------------------------
    ...
    n_idx_input = st.number_input(
        "Product/Name column (1-based; 0 = auto)", min_value=0, max_value=len(headers),
